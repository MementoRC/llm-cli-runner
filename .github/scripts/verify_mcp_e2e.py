...
class MCPVerificationRunner:
    ...
    async def run_verification(self) -> bool:
        ...
        self.log(
            "This replicates the manual verification process performed during debugging"
        )
        ...
        try:
            ...
            self.results[
                "phase_2_github_api"
            ] = await self._test_github_api_operations()

            # Phase 3: Advanced Git Operations
            self.log("Phase 3: Testing Advanced Git Operations")
            self.results[
                "phase_3_advanced_git"
            ] = await self._test_advanced_git_operations()
            ...
            # Overall assessment
            passed_phases = sum(
                self.results[key] for key in self.results if key != "overall_success"
            )
    ...
    async def _test_basic_git_operations(self) -> bool:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                result = await self._run_mcp_tool(
                    "git_log", {"repo_path": str(repo_path), "max_count": 5}
                )
                if not self._is_successful_result(result):
                ...
                result = await self._run_mcp_tool(
                    "git_diff_staged", {"repo_path": str(repo_path)}
                )
                if not self._is_successful_result(result):
        ...
        try:
            ...
            if not github_token:
                ...
                success = self._is_successful_result(
                    result
                ) or self._is_graceful_failure(result)
                if success:
                ...
                self.log(
                    "  GITHUB_TOKEN available - testing full GitHub API functionality"
                )
                ...
                if not (
                    self._is_successful_result(result)
                    or self._is_graceful_failure(result)
                ):
    ...
    async def _test_advanced_git_operations(self) -> bool:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                result = await self._run_mcp_tool(
                    "git_show", {"repo_path": str(repo_path), "revision": "HEAD"}
                )
                if not self._is_successful_result(result):
                ...
                result = await self._run_mcp_tool(
                    "git_security_validate", {"repo_path": str(repo_path)}
                )
                if not self._is_successful_result(result):
    ...
    async def _test_error_handling(self) -> bool:
        ...
        try:
            ...
            result = await self._run_mcp_tool(
                "git_status", {"repo_path": "/nonexistent/path"}
            )
            ...
            result = await self._run_mcp_tool(
                "git_status", {"repo_path": str(Path.cwd())}
            )
...
async def main():
    ...
    parser.add_argument("--github-token", type=str, help="GitHub token for API testing")

    args = parser.parse_args()

    # Run verification
    runner = MCPVerificationRunner(verbose=args.verbose, github_token=args.github_token)
