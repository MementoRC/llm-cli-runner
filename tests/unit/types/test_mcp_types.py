    ...
    def test_should_represent_successful_responses(self):
        ...
        if TYPES_AVAILABLE:
            response = MCPResponse.success(
                id=1, result={"tools": [{"name": "git_status"}]}
            )
...
class TestMCPTool:
    ...
    def test_should_validate_json_schemas(self):
        ...
        if TYPES_AVAILABLE:
            ...
            tool = MCPTool(
                name="test_tool", description="Test tool", input_schema=valid_schema
            )
    ...
    def test_should_validate_tool_inputs(self):
        ...
        if TYPES_AVAILABLE:
            ...
            valid_input = MCPToolInput(
                schema=schema, arguments={"repo_path": "/tmp/repo", "count": 5}
            )
...
class TestMCPToolOutput:
    ...
    def test_should_represent_tool_errors(self):
        ...
        if TYPES_AVAILABLE:
            output = MCPToolOutput.error(
                error_message="Repository not found", error_code="REPO_NOT_FOUND"
            )
    ...
    def test_should_validate_content_types(self):
        ...
        if TYPES_AVAILABLE:
            ...
            with pytest.raises(MCPValidationError):
                MCPToolOutput.success(content=[{"type": "invalid", "data": "test"}])
...
class TestMCPSession:
    ...
    def test_should_track_session_capabilities(self):
        ...
        if TYPES_AVAILABLE:
            ...
            assert (
                not effective.supports_resource_subscription()
