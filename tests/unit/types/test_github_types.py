    ...
    def test_should_validate_urls(self):
        ...
        if TYPES_AVAILABLE:
            ...
            with pytest.raises(GitHubValidationError):
                GitHubRepository(owner="owner", name="repo", clone_url="invalid-url")

            # Should reject invalid SSH URLs
            with pytest.raises(GitHubValidationError):
                GitHubRepository(owner="owner", name="repo", ssh_url="invalid-ssh-url")
    ...
    def test_should_provide_pull_request_predicates(self):
        ...
        if TYPES_AVAILABLE:
            open_pr = GitHubPullRequest(
                number=123, title="Test PR", state=GitHubPRState.OPEN
            )
    ...
    def test_should_wrap_error_responses(self):
        ...
        if TYPES_AVAILABLE:
            response = GitHubAPIResponse.error(
                message="Not Found", status_code=404, error_code="RESOURCE_NOT_FOUND"
            )
    ...
    def test_should_handle_rate_limiting(self):
        ...
        if TYPES_AVAILABLE:
            rate_limited = GitHubAPIResponse.rate_limited(
                reset_time=datetime.now(), remaining_requests=0, limit=5000
            )
...
class TestGitHubPagination:
    ...
    def test_should_handle_paginated_responses(self):
        ...
        if TYPES_AVAILABLE:
            pagination = GitHubPagination(
                page=2, per_page=30, total_count=150, has_next=True, has_previous=True
            )
    ...
    def test_should_calculate_pagination_metadata(self):
        ...
        if TYPES_AVAILABLE:
            pagination = GitHubPagination(page=3, per_page=25, total_count=100)
    ...
    def test_should_track_rate_limit_status(self):
        ...
        if TYPES_AVAILABLE:
            rate_limit = GitHubRateLimit(
                limit=5000, remaining=4500, reset_time=datetime.now(), used=500
            )
    ...
    def test_should_detect_rate_limit_exhaustion(self):
        ...
        if TYPES_AVAILABLE:
            exhausted_limit = GitHubRateLimit(
                limit=5000, remaining=0, reset_time=datetime.now(), used=5000
            )
...
class TestGitHubTypeIntegration:
    ...
    def test_github_types_should_work_together(self):
        ...
        if TYPES_AVAILABLE:
            ...
            pr = GitHubPullRequest(
                number=456, title="Test PR", state=GitHubPRState.OPEN, user=user
            )
