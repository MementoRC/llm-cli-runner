    ...
    def test_should_represent_successful_validation(self):
        ...
        if TYPES_AVAILABLE:
            result = ValidationResult.success(
                value="valid-data", message="Validation passed"
            )
    ...
    def test_should_categorize_validation_errors(self):
        ...
        if TYPES_AVAILABLE:
            format_error = ValidationError(
                "Invalid format", category=ValidationCategory.FORMAT
            )
            range_error = ValidationError(
                "Value out of range", category=ValidationCategory.RANGE
            )
            required_error = ValidationError(
                "Field required", category=ValidationCategory.REQUIRED
            )
    ...
    def test_should_support_error_severity_levels(self):
        ...
        if TYPES_AVAILABLE:
            warning = ValidationError(
                "Deprecated field", severity=ValidationSeverity.WARNING
            )
            error = ValidationError("Invalid value", severity=ValidationSeverity.ERROR)
            critical = ValidationError(
                "System failure", severity=ValidationSeverity.CRITICAL
            )
...
class TestPathValidator:
    ...
    def test_should_validate_file_paths(self):
        ...
        if TYPES_AVAILABLE:
            validator = PathValidator(must_exist=True, must_be_file=True, readable=True)
    ...
    def test_should_validate_directory_paths(self):
        ...
        if TYPES_AVAILABLE:
            validator = PathValidator(
                must_exist=True, must_be_directory=True, writable=True
            )
...
class TestGitHubNameValidator:
    ...
    def test_should_validate_github_repository_names(self):
        ...
        if TYPES_AVAILABLE:
            ...
            valid_repos = ["repo", "repo-name", "repo.name", "repo_name", "123-repo"]
            ...
            schema = ValidationSchema(
                {
                    ...
                    "phone": FieldValidator(
                        ValidationRule("phone", lambda x: len(x) >= 10), required=False
                    ),
                }
            )
...
class TestValidationReport:
    ...
    def test_should_generate_validation_reports(self):
        ...
        if TYPES_AVAILABLE:
            errors = [
                ValidationError(
                    "Error 1", field_name="field1", severity=ValidationSeverity.ERROR
                ),
                ValidationError(
                    "Warning 1",
                    field_name="field2",
                ...
                ValidationError(
                    "Error 2", field_name="field3", severity=ValidationSeverity.CRITICAL
                ),
            ]
