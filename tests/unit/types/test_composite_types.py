        ...
        if TYPES_AVAILABLE:
            request = GitOperationRequest(
                ...
                context=OperationContext(
                    user_id="user123", session_id="session456", timestamp=datetime.now()
                ),
            )
    ...
    def test_should_combine_git_results_with_mcp_response(self):
        ...
        if TYPES_AVAILABLE:
            ...
            response = GitOperationResponse(
                mcp_response=MCPResponse.success(id=1, result={"status": "success"}),
    ...
    def test_should_provide_response_serialization(self):
        ...
        if TYPES_AVAILABLE:
            response = GitOperationResponse(
                ...
                git_result=GitStatusResult(
                    is_clean=True, current_branch=GitBranch("main")
                ),
                operation_type="status",
            )
    ...
    def test_should_combine_github_and_mcp_request_data(self):
        ...
        if TYPES_AVAILABLE:
            request = GitHubOperationRequest(
                ...
                api_context={"pr_number": 123, "include_annotations": True},
            )
    ...
    def test_should_handle_github_authentication(self):
        ...
        if TYPES_AVAILABLE:
            request = GitHubOperationRequest(
                ...
                rate_limit_context={"remaining": 4500, "reset_time": datetime.now()},
            )
    ...
    def test_should_handle_partial_failures(self):
        ...
        if TYPES_AVAILABLE:
            ...
            github_failure = GitHubWorkflowResult(
                operations=["create_pr"], success=False, error="API rate limit exceeded"
            )
