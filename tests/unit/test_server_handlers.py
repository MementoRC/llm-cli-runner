...
from mcp.types import Tool, CallToolRequest, CallToolRequestParams, CallToolResult, TextContent
...
class TestCheapLLMServerTDD:
    ...
    async def test_list_tools_multiple_providers(self):
        ...
        mock_config_manager = Mock()
        mock_config_manager.get_enabled_providers.return_value = ["gemini", "codex", "llama"]
    ...
    async def test_call_tool_gemini(self):
        ...
        with patch.object(server, '_call_gemini', return_value="Test response") as mock_gemini:
    ...
    async def test_tool_error_handling(self):
        ...
        with patch.object(server, '_call_gemini', side_effect=RuntimeError("Test error")) as mock_gemini:
            request = CallToolRequest(
                method="tools/call",
                params=CallToolRequestParams(
                    name="gemini_generate",
