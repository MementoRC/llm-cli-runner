...
from mcp_server_git.configuration import (
    GitServerConfig,
    GitConfig,
    GitHubConfig
)
...
class TestGitConfig:
    ...
    def test_author_validation(self):
        ...
        with pytest.raises(ValidationError):
            GitConfig(allowed_authors=["Invalid <not@an@email>"])  # Invalid email format
    
    def test_dependent_settings_validation(self):
        """Test interdependent settings validation."""
        # Warning for conflicting settings
        with pytest.warns(UserWarning, match="Commit signing is enabled but force push is allowed"):
            GitConfig(
                enable_commit_signing=True,
                allow_force_push=True
            )
        
        with pytest.warns(UserWarning, match="Pull requests are required but no approvals are needed"):
            GitConfig(
...
class TestGitHubConfig:
    ...
    def test_url_normalization(self):
        ...
        assert str(enterprise_config.api_base_url) == "https://github.enterprise.com/api/v3"
