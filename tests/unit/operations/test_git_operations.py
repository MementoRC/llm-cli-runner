...
class TestCommitChangesWithValidation:
    ...
    def test_commit_with_gpg_signing(
        ...
        repo_path = Path("/test/repo")
        commit_request = CommitRequest(
            message="feat: secure feature", gpg_sign=True, gpg_key_id="ABC123"
        )
    ...
    def test_commit_fails_when_file_staging_fails(
        ...
        repo_path = Path("/test/repo")
        commit_request = CommitRequest(
            message="feat: add feature", files=["nonexistent.py"]
        )
...
class TestCreateBranchWithCheckout:
    ...
    def test_successful_branch_creation_with_checkout(
        ...
        branch_request = BranchRequest(
            name="feature/new-feature", base_branch="main", checkout=True
        )
        ...
        mock_execute_git.side_effect = [
            GitCommandError(
                "Branch not found"
            ),  # show-ref check (branch doesn't exist)
    ...
    def test_branch_creation_without_checkout(
        ...
        repo_path = Path("/test/repo")
        branch_request = BranchRequest(
            name="feature/background-task", base_branch="develop", checkout=False
        )
    ...
    def test_branch_creation_fails_when_branch_exists(
        ...
        # Arrange
        repo_path = Path("/test/repo")
        branch_request = BranchRequest(name="existing-branch", force=False)
    ...
    @patch("mcp_server_git.operations.git_operations.validate_repository_path")
    @patch("mcp_server_git.operations.git_operations.get_current_branch")
    def test_branch_creation_fails_with_empty_name(
        self, mock_get_current_branch, mock_validate_repo
    ):
...
class TestMergeBranchesWithConflictDetection:
    ...
    def test_merge_detects_conflicts(
        ...
        repo_path = Path("/test/repo")
        merge_request = MergeRequest(
            source_branch="feature/conflicting-feature", target_branch="main"
        )
    ...
    def test_merge_with_squash_option(
        ...
        # Arrange
        repo_path = Path("/test/repo")
        merge_request = MergeRequest(source_branch="feature/small-feature", squash=True)
...
class TestPushWithValidation:
    ...
    def test_push_with_upstream_setting(
        ...
        result = push_with_validation(
            repo_path, remote="origin", branch="feature/new-branch", set_upstream=True
        )
...
def sample_branch_request():
    ...
    return BranchRequest(name="test/sample-branch", base_branch="main", checkout=True)
