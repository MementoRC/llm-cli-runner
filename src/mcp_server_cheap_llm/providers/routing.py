...
from ..core.models import LLMRequest, ProviderType
...
class ComplexityAnalyzer:
    ...
    def __init__(self):
        ...
        self._context_regex = re.compile(
            "|".join(self.CONTEXT_INDICATORS), re.IGNORECASE
        )
    ...
    def extract_features(self, prompt: str) -> ComplexityFeatures:
        ...
        question_words = len(
            re.findall(
                r"\b(?:what|how|why|when|where|which|who)\b", prompt, re.IGNORECASE
            )
        )
    ...
    def calculate_complexity_score(self, features: ComplexityFeatures) -> float:
        ...
        normalized_score = min(10.0, (score / 23.0) * 10.0)
...
class ProviderRouter:
    ...
    def route_request(self, request: LLMRequest) -> RoutingDecision:
        ...
        if request.provider:
            if self._is_provider_available(request.provider):
                ...
                decision = RoutingDecision(
                    ...
                    fallback_providers=self._get_fallback_providers(request.provider),
                    features=features,
    ...
    def _calculate_confidence(
        self, score: float, level: ComplexityLevel, provider: ProviderType
    ) -> float:
        ...
        if level == ComplexityLevel.SIMPLE and score > 2.5:
            base_confidence *= 0.9
        elif level == ComplexityLevel.MODERATE and (score < 3.2 or score > 4.8):
            base_confidence *= 0.9
        elif level == ComplexityLevel.COMPLEX and (score < 5.2 or score > 6.8):
            base_confidence *= 0.9
        elif level == ComplexityLevel.EXPERT and score < 7.2:
            base_confidence *= 0.9
    ...
    def _log_routing_decision(self, decision: RoutingDecision, request: LLMRequest):
    ...
    def _track_routing_decision(self, decision: RoutingDecision, request: LLMRequest):
