...
This module provides caching functionality for the cheap-llm server,
...
class RequestCache:
    """Cache specifically for request/response pairs."""
    ...
    async def cache_response(
        ...
        """
        key = self._generate_request_key(request_data)
        return await self._cache.set(key, response_data, ttl)

    async def invalidate_request(self, request_data: Any) -> bool:
