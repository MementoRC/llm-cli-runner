"""Custom exceptions for MCP Server LLM CLI Runner.
...
Key exceptions:
    LLMCliRunnerError: Base exception class
...
class LLMCliRunnerError(Exception):
    """Base exception class for MCP Server LLM CLI Runner.
    ...
    Example:
        >>> try:
        ...     raise LLMCliRunnerError("Something went wrong", error_code="E001")
        ... except LLMCliRunnerError as e:
...
class ConfigurationError(LLMCliRunnerError):
...
class ProviderError(LLMCliRunnerError):
...
class SecurityError(LLMCliRunnerError):
