...
class InputSanitizer:
    ...
    def sanitize_repository_path(path: str) -> str:
        ...
        if ".." in normalized or normalized.startswith("/"):
            raise GitValidationError(
                ...
                suggested_fix="Use relative paths within the working directory"
            )
    ...
    def validate_file_path(file_path: str) -> bool:
        ...
        if path.suffix and path.suffix.lower() not in SecurityDefaults.ALLOWED_EXTENSIONS:
            return False
...
class SecurityFramework(DebuggableComponent):
    ...
    def validate_component(self) -> ValidationResult:
        ...
        current_failures = sum(len(attempts) for attempts in self.failed_attempts.values())
    ...
    def authenticate_github_token(self, token: Optional[str] = None) -> AuthResult:
        ...
        if validation_result.status in [SecurityStatus.CRITICAL, SecurityStatus.INSECURE]:
    ...
    def validate_repository_access(self, repository_path: str) -> SecurityValidationResult:
        ...
        try:
            ...
            sanitized_path = self.input_sanitizer.sanitize_repository_path(repository_path)
        ...
        return SecurityValidationResult(
            ...
            metadata={
                ...
                "sanitized_path": sanitized_path if 'sanitized_path' in locals() else None,
                "path_exists": repo_path.exists() if 'repo_path' in locals() else False
            }
        )
    
    def validate_git_operation(self, operation: str, params: Dict[str, Any]) -> SecurityValidationResult:
        ...
        if operation == "commit":
            ...
            try:
                sanitized_message = self.input_sanitizer.sanitize_commit_message(message)
                if sanitized_message != message:
                    issues.append(SecurityIssue(
                        severity=SecuritySeverity.MEDIUM,
                        category=SecurityCategory.INPUT_VALIDATION,
                        message="Commit message was sanitized for security",
                        context={"original": message, "sanitized": sanitized_message}
                    ))
            except GitValidationError as e:
                issues.append(SecurityIssue(
                    severity=SecuritySeverity.HIGH,
        ...
        self._log_security_event("operation_validation", {
            "operation": operation,
            "status": status.value,
    ...
    def _check_rate_limit(self, key: str, limit: int = 60, window: int = 60, check_only: bool = False) -> bool:
    ...
    def _record_failed_attempt(self, context: str, reason: str):
        ...
        self.failed_attempts[context] = [
            attempt for attempt in self.failed_attempts[context]
            if attempt > cutoff
        ]
