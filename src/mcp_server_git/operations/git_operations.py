        ...
        if commit_request.files:
            for file_path in commit_request.files:
                try:
                    ...
                    return CommitResult(
                        success=False,
                        error=f"Failed to stage file {file_path}: {e}"
                    )
...
def create_branch_with_checkout(
    ...
    """
    try:
        logger.info(f"Creating branch '{branch_request.name}' in repository: {repo_path}")
        ...
        try:
            execute_git_command(["show-ref", "--verify", f"refs/heads/{branch_request.name}"], repo_path_str)
        ...
        if branch_request.base_branch:
            branch_cmd = ["checkout", "-b", branch_request.name, branch_request.base_branch]
        else:
...
def merge_branches_with_conflict_detection(
    ...
    """
    try:
        logger.info(f"Merging '{merge_request.source_branch}' in repository: {repo_path}")
        ...
        if merge_request.target_branch and merge_request.target_branch != current_branch:
            try:
                execute_git_command(["checkout", merge_request.target_branch], repo_path_str)
        ...
        try:
            execute_git_command(["show-ref", "--verify", f"refs/heads/{merge_request.source_branch}"], repo_path_str)
        except GitCommandError:
            return MergeResult(
                success=False,
        ...
        try:
            ...
            if "conflict" in str(e).lower() or "merge conflict" in str(e).lower():
                ...
                try:
                    ...
                    logger.warning(f"Merge conflicts detected in files: {conflict_files}")
                    return MergeResult(
                        success=False,
        ...
        if not merge_request.squash:  # Squash merges don't create merge commits immediately
            try:
