...
def enforce_secure_git_config(repo: Repo, strict_mode: bool = True) -> str:
    ...
    try:
        ...
        if not user_email:
            # Try to get from environment or system git config
            env_email = os.getenv("GIT_AUTHOR_EMAIL") or os.getenv(
                "GIT_COMMITTER_EMAIL"
            )
            if env_email:
        ...
        if not signing_key:
            ...
            if env_key:
                ...
                changes_made.append(
                    f"Set GPG signing key to '{env_key}' from environment"
                )
            else:
                        ...
                        for line in lines:
                            if line.strip().startswith("sec"):
                                ...
                                if len(parts) >= 2 and "/" in parts[1]:
                                    ...
                                    changes_made.append(
                                        f"Auto-detected and set GPG signing key: {key_id}"
                                    )
                                    break
                        else:
                            if strict_mode:
