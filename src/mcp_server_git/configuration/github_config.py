...
class GitHubConfig(BaseModel):
    ...
    api_version: str = Field(
        default="2022-11-28",
        description="GitHub API version (use YYYY-MM-DD format)"
    )
    ...
    validate_token_on_startup: bool = Field(
        default=True,
        description="Validate token and scopes at startup"
    )
    ...
    auto_retry_rate_limited: bool = Field(
        default=True,
        description="Automatically retry requests when rate limited"
    )
    ...
    validate_webhook_payload: bool = Field(
        default=True,
        description="Enable webhook payload signature validation"
    )
    
    # PR and Issue Settings
    auto_link_issues: bool = Field(
        default=True,
        description="Automatically link mentioned issues in PRs"
    )
    require_issue_in_pr: bool = Field(
        default=False,
        description="Require issue reference in PR description"
    )
    allowed_pr_labels: List[str] = Field(
        default_factory=list,
        description="Allowed labels for PRs (empty = all allowed)"
    )
    auto_close_stale_prs: bool = Field(
        default=False,
        description="Automatically close stale PRs"
    )
    ...
    is_enterprise: bool = Field(
        default=False,
        description="Whether using GitHub Enterprise"
    )
    enterprise_api_path: str = Field(
        default="/api/v3",
        description="API path for GitHub Enterprise"
    )
    ...
    def validate_webhook_events(cls, v: List[str]) -> List[str]:
        ...
        valid_events = {
            ...
            'code_scanning_alert', 'commit_comment', 'create', 'delete',
            'deployment', 'deployment_status', 'discussion', 'discussion_comment',
            'fork', 'gollum', 'issue_comment', 'issues', 'label',
            ...
            'push', 'registry_package', 'release', 'repository',
            'repository_dispatch', 'secret_scanning_alert', 'star',
    ...
    def validate_token_scopes(cls, v: List[str]) -> List[str]:
        ...
        valid_scopes = {
            ...
            'gist', 'notifications', 'user', 'read:user', 'user:email',
            'user:follow', 'delete_repo', 'write:discussion',
            'read:discussion', 'write:packages', 'read:packages',
            'delete:packages', 'admin:gpg_key', 'write:gpg_key',
        ...
        if invalid_scopes:
            raise ValueError(
                f"Invalid token scopes: {', '.join(invalid_scopes)}"
            )
