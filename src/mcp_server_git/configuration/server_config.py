...
class GitServerConfig(BaseModel):
    ...
    enable_security_validation: bool = Field(
        default=True,
        description="Enable security validation for Git operations"
    )
    ...
    require_gpg_signing: bool = Field(
        default=False,
        description="Require GPG signing for all commits"
    )
    ...
    github_rate_limit_buffer: int = Field(
        ...
        ge=10, 
        le=1000,
        description="Buffer for GitHub API rate limits"
    )
    ...
    def validate_forbidden_operations(cls, v: List[str]) -> List[str]:
        ...
        """
        valid_operations = {
            'push', 'pull', 'fetch', 'commit', 'merge', 'rebase',
            'cherry-pick', 'reset', 'revert', 'force-push', 'force-pull',
            'branch-delete', 'tag-delete', 'stash', 'clean'
        }
