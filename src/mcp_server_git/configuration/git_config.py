...
class GitConfig(BaseModel):
    ...
    allow_force_push: bool = Field(
        default=False, description="Allow force push operations"
    )
    allow_submodules: bool = Field(
        default=True, description="Allow operations on repositories with submodules"
    )
    ...
    max_file_size_mb: int = Field(
        default=100, ge=1, le=1000, description="Maximum file size in MB for operations"
    )
    max_diff_size_mb: int = Field(
        default=50, ge=1, le=500, description="Maximum diff size in MB"
    )
    ...
    diff_context_lines: int = Field(
        default=3, ge=0, le=20, description="Number of context lines to show in diffs"
    )
    ...
    require_issue_reference: bool = Field(
        default=False, description="Require issue references in commit messages"
    )
    ...
    allow_delete_branch: bool = Field(
        default=True, description="Allow branch deletion operations"
    )
    require_pull_request: bool = Field(
        default=False, description="Require pull requests for protected branches"
    )
    ...
    enable_git_cache: bool = Field(
        default=True, description="Enable Git object caching for performance"
    )
    cache_size_mb: int = Field(
        default=256, ge=32, le=2048, description="Git cache size in MB"
    )
    parallel_operations: int = Field(
        default=4, ge=1, le=16, description="Number of parallel Git operations allowed"
    )
    operation_queue_size: int = Field(
        default=100, ge=10, le=1000, description="Maximum number of queued operations"
    )
