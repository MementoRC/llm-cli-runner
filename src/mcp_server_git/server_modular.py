        ...
        try:
            ...
            if name not in [
                ...
                match name:
                    ...
                    case GitTools.ADD:
                        add_result = (
                            modular_git_add(repo, arguments["files"])
                            if USE_MODULAR_GIT
                            else git_add(repo, arguments["files"])
                        )
                        result = [TextContent(type="text", text=add_result)]

                    case GitTools.RESET:
                        reset_result = (
                            modular_git_reset(repo)
                            if USE_MODULAR_GIT
                            else git_reset(repo)
                        )
                    ...
                    case GitTools.LOG:
                        ...
                        result = [
                            TextContent(type="text", text=f"Commit log:\n{log_result}")
                        ]
                    ...
                    match name:
                        case GitTools.GITHUB_GET_PR_DETAILS:
                            ...
                            if not repo_owner or not repo_name:
                                ...
                                result = [
                                    TextContent(type="text", text=pr_details_result)
                                ]
                        ...
                        case _:
                            result = [
                                TextContent(
                                    type="text",
                                    text=f"❌ Original GitHub tool '{name}' not implemented in modular fallback",
                                )
                            ]
