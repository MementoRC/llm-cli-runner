...
class ComponentStateInspector:
    ...
    def unregister_component(self, component_id: str) -> bool:
        ...
        with self._lock:
            ...
            if component_id in self._component_metadata:
                self._component_metadata[component_id]['unregistration_time'] = datetime.now()
            return removed
    ...
    def capture_state_snapshot(self, component_id: str) -> StateSnapshot | None:
        ...
        with self._lock:
            ...
            try:
                ...
                self._component_metadata[component_id]['last_snapshot_time'] = snapshot.snapshot_timestamp
    ...
    def get_state_history(self, component_id: str, limit: int = 10) -> list[StateSnapshot]:
        """
    ...
    def compare_states(self, component_id: str, snapshot1: StateSnapshot, snapshot2: StateSnapshot) -> dict[str, Any]:
        """
        ...
        comparison = {
            ...
            'time_delta_seconds': (snapshot2.snapshot_timestamp - snapshot1.snapshot_timestamp).total_seconds(),
            'differences': {},
            'summary': {},
        }
        ...
        comparison['differences'] = {
            ...
            'changes': {key: {'old': state1[key], 'new': state2[key]} for key in changed_keys},
        }
        ...
        for metric in set(perf1.keys()) | set(perf2.keys()):
            ...
            if old_val != new_val:
                perf_changes[metric] = {
                    ...
                    'percent_change': ((new_val - old_val) / old_val * 100) if old_val != 0 else float('inf')
                }
        ...
        # Summary
        comparison['summary'] = {
            'has_changes': bool(added_keys or removed_keys or changed_keys or validation_changed or perf_changes),
    ...
    def generate_llm_friendly_report(self, component_id: str | None = None) -> str:
        ...
        with self._lock:
            ...
            if component_id:
                components_to_report = [component_id] if component_id in self._components else []
            else:
            ...
            report_lines.append(f"Components in this report: {len(components_to_report)}")
            report_lines.append("")
            ...
            for comp_id in components_to_report:
                ...
                report_lines.append(f"**Type**: {metadata.get('component_type', 'Unknown')}")
                report_lines.append(f"**Registered**: {metadata.get('registration_time', 'Unknown')}")
                report_lines.append(f"**Snapshots captured**: {metadata.get('snapshot_count', 0)}")

                # Current state snapshot
                current_snapshot = self.capture_state_snapshot(comp_id)
                if current_snapshot:
                    report_lines.append("### Current State")
                    report_lines.append(f"**Timestamp**: {current_snapshot.snapshot_timestamp.isoformat()}")
                    ...
                    report_lines.append(f"**State data keys**: {list(state_data.keys())}")

                    # Recent history summary
                    history = self.get_state_history(comp_id, limit=5)
                    if len(history) > 1:
                        report_lines.append(f"**Recent history**: {len(history)} snapshots available")

                        # Compare with previous state
                        if len(history) >= 2:
                            comparison = self.compare_states(comp_id, history[1], history[0])
                            if comparison['summary']['has_changes']:
                                ...
                                if changes['added_keys']:
                                    report_lines.append(f"  - Added keys: {changes['added_keys']}")
                                if changes['removed_keys']:
                                    report_lines.append(f"  - Removed keys: {changes['removed_keys']}")
                                if changes['changed_keys']:
                                    report_lines.append(f"  - Changed keys: {changes['changed_keys']}")
                            else:
            ...
            report_lines.append("")
            report_lines.append("---")
            report_lines.append("*This report is optimized for LLM analysis and human debugging*")
        ...
        with self._lock:
            ...
            for comp_id, history in self._state_history.items():
                export_data['state_history_summary'][comp_id] = {
                    ...
                    'oldest_snapshot': history[0].snapshot_timestamp.isoformat() if history else None,
                    'newest_snapshot': history[-1].snapshot_timestamp.isoformat() if history else None,
                }
