    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            'duration_stats': {
                ...
                'std_dev': statistics.stdev(self.durations) if len(self.durations) > 1 else 0.0,
                'percentile_95': statistics.quantiles(self.durations, n=20)[18] if len(self.durations) >= 20 else (self.max_duration if self.durations else 0.0),
            }
        }
...
class PerformanceProfiler:
    ...
    def __init__(self, max_history_per_operation: int = 1000, enable_resource_monitoring: bool = True):
        """
        ...
        self._metrics_history: dict[str, deque] = defaultdict(lambda: deque(maxlen=max_history_per_operation))
    ...
    def capture_resource_snapshot(self) -> ResourceSnapshot | None:
        ...
        try:
            ...
            if self._initial_net:
                net_sent_mb = (current_net.bytes_sent - self._initial_net.bytes_sent) / (1024 * 1024)
                net_recv_mb = (current_net.bytes_recv - self._initial_net.bytes_recv) / (1024 * 1024)
            else:
    ...
    @contextmanager
    def profile_operation(self, operation_name: str, operation_id: str | None = None) -> Iterator[dict[str, Any]]:
        """
        ...
        try:
            ...
            self._record_operation(operation_name, duration, error_occurred, operation_metadata)

            # Remove from active operations
            with self._lock:
                self._active_operations.pop(op_id, None)

    def _record_operation(self, operation_name: str, duration: float, error_occurred: bool, metadata: dict[str, Any]) -> None:
    ...
    def record_custom_metric(self, metric_name: str, value: float, unit: str = "", metadata: dict[str, Any] | None = None) -> None:
        """
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            total_operations = sum(profile.total_calls for profile in self._operation_profiles.values())
            total_errors = sum(profile.errors for profile in self._operation_profiles.values())
            ...
            if resource_snapshots:
                avg_cpu = statistics.mean(snap.cpu_percent for snap in resource_snapshots)
                avg_memory = statistics.mean(snap.memory_percent for snap in resource_snapshots)
            ...
            return {
                ...
                'top_operations_by_duration': [op.to_dict() for op in operations_by_duration],
            }

    def generate_performance_report(self, include_detailed_history: bool = False) -> str:
        """
        ...
        report_lines.append(f"**Uptime**: {summary['uptime_seconds']:.1f} seconds ({summary['uptime_seconds']/3600:.1f} hours)")
        ...
        report_lines.append(f"**Operations/Second**: {summary['operations_per_second']:.2f}")
        report_lines.append(f"**Unique Operation Types**: {summary['unique_operation_types']}")
        report_lines.append(f"**Active Operations**: {summary['active_operations_count']}")
        report_lines.append("")

        # Resource usage
        if summary['resource_monitoring_enabled']:
            report_lines.append("## Resource Usage")
            report_lines.append(f"**Average CPU**: {summary['average_cpu_percent']:.1f}%")
            report_lines.append(f"**Average Memory**: {summary['average_memory_percent']:.1f}%")
            ...
            if summary['current_resources']:
                ...
                report_lines.append(f"- Available Memory: {current['memory_available_mb']:.1f} MB")
                report_lines.append(f"- Open Files: {current['open_files']}")
                report_lines.append(f"- Threads: {current['threads']}")
                report_lines.append(f"- Disk I/O: {current['disk_io_read_mb']:.1f} MB read, {current['disk_io_write_mb']:.1f} MB write")
                report_lines.append(f"- Network I/O: {current['network_sent_mb']:.1f} MB sent, {current['network_recv_mb']:.1f} MB received")
        else:
            report_lines.append("## Resource Usage")
        ...
        if summary['top_operations_by_calls']:
            ...
            for i, op in enumerate(summary['top_operations_by_calls'], 1):
                ...
                report_lines.append(f"- **Avg Duration**: {op['avg_duration']*1000:.2f}ms")
        ...
        if summary['top_operations_by_duration']:
            ...
            for i, op in enumerate(summary['top_operations_by_duration'], 1):
                report_lines.append(f"### {i}. {op['operation_name']}")
                report_lines.append(f"- **Total Duration**: {op['total_duration']:.2f}s")
                report_lines.append(f"- **Calls**: {op['total_calls']:,}")
                report_lines.append(f"- **Avg Duration**: {op['avg_duration']*1000:.2f}ms")
                report_lines.append(f"- **Min/Max**: {op['min_duration']*1000:.2f}ms / {op['max_duration']*1000:.2f}ms")
                if 'duration_stats' in op:
                    stats = op['duration_stats']
                    report_lines.append(f"- **Median**: {stats['median']*1000:.2f}ms")
                    report_lines.append(f"- **95th Percentile**: {stats['percentile_95']*1000:.2f}ms")
                    report_lines.append(f"- **Std Dev**: {stats['std_dev']*1000:.2f}ms")
                report_lines.append("")
                ...
                for operation_name, metrics in self._metrics_history.items():
                    if metrics:
                        ...
                        for metric in recent_metrics:
                            report_lines.append(f"- {metric.timestamp.isoformat()}: {metric.value:.3f} {metric.unit}")
                        report_lines.append("")
        ...
        if error_rate > 5.0:
            report_lines.append("🔴 **High Error Rate**: Error rate exceeds 5%. Investigate failing operations.")
        elif error_rate > 1.0:
            report_lines.append("🟡 **Elevated Error Rate**: Error rate above 1%. Monitor for trends.")
        else:
        ...
        if summary['resource_monitoring_enabled']:
            ...
            if avg_cpu > 80:
                report_lines.append("🔴 **High CPU Usage**: Average CPU usage above 80%. Consider optimization.")
            elif avg_cpu > 60:
                report_lines.append("🟡 **Moderate CPU Usage**: Average CPU usage above 60%. Monitor trends.")

            if avg_memory > 80:
                report_lines.append("🔴 **High Memory Usage**: Average memory usage above 80%. Check for memory leaks.")
            elif avg_memory > 60:
                report_lines.append("🟡 **Moderate Memory Usage**: Average memory usage above 60%. Monitor trends.")

        # Check for slow operations
        with self._lock:
            slow_operations = []
            for profile in self._operation_profiles.values():
                if profile.avg_duration > 1.0:  # Operations taking more than 1 second on average
                    slow_operations.append(profile)

        if slow_operations:
            report_lines.append("🔍 **Slow Operations Detected**:")
            for op in sorted(slow_operations, key=lambda x: x.avg_duration, reverse=True)[:3]:
                report_lines.append(f"  - {op.operation_name}: {op.avg_duration:.2f}s average")

        report_lines.append("")
        report_lines.append("---")
        report_lines.append("*This report is optimized for LLM analysis and performance optimization*")
